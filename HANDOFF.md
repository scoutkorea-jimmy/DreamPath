...
- `admin.html` 572KB → 41KB. 나머지는 `admin-1-core-content.js` ~ `admin-4-apps-shell.js`.
  **내용은 한 글자도 안 바꿨다** — 이어 붙이면 원본과 바이트 단위로 같다(511,180자 확인).
- ⚠️ **네 파일은 순서대로 실행돼야 한다.** 뒤 파일이 앞 파일의 선언을 쓴다. 순서를
  바꾸거나 하나를 빼면 관리자 화면이 통째로 뜨지 않는다.
- ⚠️ **확장자가 `.js` 인 이유는 압축**이다. `text/jsx` 는 Cloudflare 압축 대상이 아니라
  나눈 직후 전송량이 124KB → 649KB 로 뛰었다. `.js` 는 `text/javascript` 로 나가 brotli
  가 걸린다(154KB). Babel 은 확장자가 아니라 script 태그의 `type` 을 본다.
- ⚠️ worker 에서 content-type 만 고치는 방법은 **닿지 않는다** — Workers Assets 는 자산이
  존재하면 Worker 를 아예 거치지 않는다.
- **첫 방문 전송은 30KB 늘었다**(조각별 압축이 통짜보다 41% 비효율). 얻은 것은 500KB
  경고 해소 · 마운트 9% 단축 · 캐시 분리.
- 검사기 셋(`jsx-check`·preflight 이모지·`build_inventory.py`)이 확장자로 대상을 골라
  **조용히 건너뛰었다**(탭 50 → 0). 셋 다 HTML 실제 로드 목록 기반으로 고쳤다.
- ⚠️ **남은 것**: 공개 사이트 `.jsx` 21개는 같은 압축 문제를 그대로 안고 있다(홈 501KB).
