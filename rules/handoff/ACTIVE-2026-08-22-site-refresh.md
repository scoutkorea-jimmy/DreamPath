...
- [ ] 규칙 체계 구축 (1~6번) — CLAUDE.md 개편 + `rules/` 신설
- [ ] 위키(`wiki:versions` · `wiki:kms`) 갱신
- [ ] 루트 `HANDOFF.md` 갱신
...
- [ ] **← 지금 여기**: `CLAUDE.md` 개편(라우팅표 + 새 절차) → 위키/HANDOFF 갱신
...
## 종료 시 할 일
- [ ] 배포 → 커밋 → 푸시 (규칙 문서는 배포 대상 아님 — `.assetsignore` 에 `rules/` 추가)
- [ ] 루트 `HANDOFF.md` + 관리자 위키 갱신
- [ ] `rules/03-history-success.md` 기록 (이미 초안 있음)
