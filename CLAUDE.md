...
### D. 배포 · 커밋 자동
0. **배포는 `node rules/tools/deploy.mjs` 한 명령으로 한다.**
   이 래퍼가 **preflight → `wrangler deploy` → 배포 후 렌더 스모크**를 묶어서 돈다.
   맨손 `npx wrangler deploy` 는 쓰지 않는다 — 앞뒤 검사가 통째로 빠진다.
   - `preflight` 가 막으면 **배포하지 않는다.**
   - 스모크는 실제 브라우저로 `/admin` · `/` 를 열어 **화면이 그려지는지** 본다
     (약 7초). v01.101.10 의 백지는 배포 성공 · HTTP 200 · 파일 일치 · 구문검사
     통과를 **전부 뚫고** 나갔다 — 실행해 보는 것 외에 잡을 방법이 없었다.
   - 스모크가 실패해도 **자동으로 되돌리지 않는다.** 스모크가 틀렸을 수도 있으므로
     브라우저로 직접 확인한 뒤 사람이 `npx wrangler rollback` 을 판단한다.
   - Chrome 이 없어 스모크를 못 돌리면 종료코드 **2** 로 끝난다 — 실패(1)와 다르다.
     **못 돌린 것은 통과가 아니다.**
0-1. **배포 전 `node rules/tools/preflight.mjs`** (래퍼가 자동으로 부르지만, 편집 중에
   따로 돌려도 된다). 실패가 하나라도 있으면 배포하지 않는다. 이 검사는 **같은 함정에 세 번 걸린 뒤**(8/22 배너 ·
...
### D-2. 배포 전 3종 확인 (동시 작업 사고 이후)
1. `node rules/tools/deploy.mjs` 로 배포 — preflight 를 통과해야 나가고, 나간 뒤
   화면이 실제로 뜨는 것까지 확인한다
2. `git status --short` — **내가 만지지 않은 변경**이 있으면 다른 세션의 작업이다.
   배포는 작업트리 전체를 올리므로 남의 미완성 코드가 함께 나간다. 있으면 운영자에게 알린다.
3. `version.js` 를 올리기 전에 **위키 최신 버전 번호를 먼저 조회**한다
   (`wiki:versions` 의 첫 페이지). 같은 번호를 두 라운드가 쓰면 버전 기록이 거짓이 된다.
...
6-1. **🔴 이모지 금지.** 사용자에게 보이는 곳(화면·메일·알림)에 컬러 그림문자를
   넣지 않는다. 의미가 필요하면 **lucide 아이콘**, 강조가 필요하면 **토큰 색 +
   말로 쓴 라벨**. `→ ▶ ✓ ★ ⛶` 같은 **흑백 타이포 기호는 이모지가 아니다** —
   구분은 이름이 아니라 성질(컬러로 렌더되는가)로 한다. preflight 가 막는다.
   사유는 `rules/05-do-not.md` (2026-08-27).
